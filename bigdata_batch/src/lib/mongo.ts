...
async function mgConnectToColl(collection_name: string = 'myDocu', db_name: string = 'nodejsTest') {
...
async function readDocuFromColl(query: Object, db_name: string = 'nodejsTest', collection_name: string = 'myDocu') {
    const c = await mgConnectToColl(db_name, collection_name);
...
export {client, ObjectId,
    mgConnectToColl, readDocuFromColl, insertDocuFromC, delDocuFromC
}